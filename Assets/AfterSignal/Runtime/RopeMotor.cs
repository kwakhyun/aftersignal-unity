...
namespace AfterSignal
{
    public sealed class RopeMotor : MonoBehaviour
    {
        ...
        {
            player=owner;
            line=gameObject.AddComponent<LineRenderer>(); line.positionCount=13; line.widthMultiplier=.037f;
            line.sharedMaterial=Resources.Load<Material>("Materials/CyanFX"); line.numCapVertices=3;
            line.startColor=Color.white;line.endColor=new Color(.1f,.8f,1);line.enabled=false;
        }
        public GrappleAnchor Select(Vector2 pointer)
        {
            var cam=Camera.main; if(!cam)return null;
            float score=float.MaxValue; GrappleAnchor best=null;
            foreach(var anchor in GrappleAnchor.All) {
                ...
                if(dist>player.Tuning.ropeRange||dist<1)continue;
                ...
                if(pixels>player.Tuning.aimAssistPixels*Screen.height/900f)continue;
                if(Physics.Linecast(player.Shoulder,anchor.transform.position,1,QueryTriggerInteraction.Ignore))continue;
                float next=pixels+dist*.8f;
                if(next<score){score=next;best=anchor;}
            }
            return best;
        }
        public void Tick(ControlFrame input,float dt)
        {
            ...
            if(!input.grapple&&previousHeld) Release();
            previousHeld=input.grapple;
            if(!Attached) return;
            if(Physics.Linecast(player.Shoulder,Target.transform.position,1,QueryTriggerInteraction.Ignore)){Release();return;}
            Length=Mathf.Clamp(Length-(input.move.y*player.Tuning.ropeReelSpeed+1.6f)*dt,1.3f,player.Tuning.ropeRange);
            ...
            player.Velocity=Vector3.ClampMagnitude(v,24f);
            if(input.jump){player.Velocity+=Vector3.up*5.5f;Release();}
        }
        public void Attach(GrappleAnchor anchor)
        {
            if(!anchor||Vector3.Distance(player.Shoulder,anchor.transform.position)>player.Tuning.ropeRange)return;
            Target=anchor;Length=Vector3.Distance(player.Shoulder,anchor.transform.position)*.94f;AttachCount++;
            player.Velocity+=(anchor.transform.position-player.Shoulder).normalized*3f+Vector3.up*5f;
            player.Director.OnAnchor(anchor);SignalEffects.Burst(anchor.transform.position,SignalEffects.Cyan,14,3);
        ...
        void LateUpdate()
        {
            if(!line)return; line.enabled=Attached;
            if(!Attached)return;
        ...
        void OnDisable(){Release();previousHeld=false;}
    }
}
