...
{
    public enum DistrictLayout { Street, Escalator, Canal, Lift, Exterior, Facade }
    public sealed class DistrictSpec
    {
        public readonly StageId id;public readonly string title,theme,brief,record;public readonly int chapter,enemies;public readonly float length;public readonly DistrictLayout layout;public readonly bool glass;
        public DistrictSpec(StageId id,string title,string theme,int chapter,DistrictLayout layout,int enemies,float length,bool glass,string brief,string record){this.id=id;this.title=title;this.theme=theme;this.chapter=chapter;this.layout=layout;this.enemies=enemies;this.length=length;this.glass=glass;this.brief=brief;this.record=record;}
    ...
    public static class CampaignCatalog
    {
        ...
        public static DistrictSpec Get(StageId id){if(id==StageId.Breach)return FacadeMission;foreach(var s in Districts)if(s.id==id)return s;return null;}
        ...
        public static int NextChapter{get{int bits=PlayerPrefs.GetInt("AFTERSIGNAL.Unity.Expansion.Chapters",0);return (bits&4)==0?2:(bits&8)==0?3:(bits&16)==0?4:0;}}
        public static StageId ChapterStart(int chapter)=>chapter==2?StageId.Market:chapter==3?StageId.Skyline:StageId.Aqueduct;
        public static void Complete(int chapter){string key="AFTERSIGNAL.Unity.Expansion.Chapters";PlayerPrefs.SetInt(key,PlayerPrefs.GetInt(key,0)|(1<<chapter));PlayerPrefs.Save();}
        public static int Jobs{get=>PlayerPrefs.GetInt("AFTERSIGNAL.Unity.Expansion.Jobs",0);set{PlayerPrefs.SetInt("AFTERSIGNAL.Unity.Expansion.Jobs",value);PlayerPrefs.Save();}}
    }
}
