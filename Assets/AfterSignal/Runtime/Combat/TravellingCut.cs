...
namespace AfterSignal
{
    public sealed class TravellingCut : MonoBehaviour
    {
        GameDirector game;
        Vector3 direction;
        float damage, travel, clock;
        readonly HashSet<EnemyBrain> struck = new HashSet<EnemyBrain>();
        readonly HashSet<BreakableGlass> glassHit = new HashSet<BreakableGlass>();
        readonly HashSet<WorldActor> residentsHit = new HashSet<WorldActor>();
        public void Initialize(GameDirector director, Vector3 forward, float amount)
        {
            game = director;
            direction = forward;
            damage = amount;
            ...
            foreach (var enemy in game.Enemies)
                if (enemy && enemy.Alive && !struck.Contains(enemy))
                {
                    ...
                    if (Mathf.Abs(d.x) < 1.2f + step && Mathf.Abs(d.z) < 1.15f && Mathf.Abs(d.y) < 1.8f)
                    {
                        ...
                        enemy.React(.3f, .035f, direction);
                        SignalEffects.Impact(enemy.transform.position + Vector3.up, direction, SignalEffects.Cyan, .9f);
                        game.Audio.Play("blade_hit", enemy.transform.position, .34f, 3);
                    }
                }
