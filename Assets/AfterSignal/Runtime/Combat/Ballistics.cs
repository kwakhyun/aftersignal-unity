...
namespace AfterSignal
{
    public static class Ballistics
    {
        ...
        public static bool Cast(Vector3 origin, Vector3 direction, float range, Transform owner, out RaycastHit closest)
        {
            closest=default; float distance=range+1;
            foreach(var hit in Physics.RaycastAll(origin,direction,range,(1<<0)|(1<<9),QueryTriggerInteraction.Collide))
            {
                var c=hit.collider;
