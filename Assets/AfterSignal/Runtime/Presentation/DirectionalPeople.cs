...
namespace AfterSignal
{
    public static class PeopleArt
    {
        ...
        public static Sprite Get(string key,int facing,int phase=0){if(key.StartsWith("Facility"))return FacilityPeople.Get(key,facing);var s=Sheet(key);return s.Length>=16?s[Mathf.Clamp(facing,0,3)*4+Mathf.Clamp(phase,0,3)]:null;}
    ...
    public sealed class DirectionalPerson:MonoBehaviour
    {
        ...
        SpriteRenderer visual;Vector3 previous;float phase;int facing;
        ...
        void LateUpdate()
        {
            ...
            var body=GetComponent<WorldActor>();if(body&&!body.Alive)return;
            ...
            if(sprite){visual.sprite=sprite;visual.flipX=FacilityPeople.Flip(art,facing);visual.color=Color.white;}
