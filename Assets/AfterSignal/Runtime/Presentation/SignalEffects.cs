...
namespace AfterSignal
{
    public static class SignalEffects
    {
        ...
        public static void Beam(Vector3 from, Vector3 to, Color color, float width, float life)
        {
            ...
            var fade = go.AddComponent<TransientEffect>();
            fade.life = life;
            fade.line = line;
        }
        ...
        public static void Ring(Vector3 center, Color color, float radius, float life)
        {
            ...
            for (int i = 0; i < 65; i++)
            {
                ...
                line.SetPosition(i, center + new Vector3(Mathf.Cos(a), Mathf.Sin(a), 0) * radius);
            }
        ...
        public static void Slash(Vector3 center,Vector3 heading,float radius,Color color,int combo)
        {
            ...
            var go = new GameObject("Blade arc");
            var line = go.AddComponent<LineRenderer>();
            line.sharedMaterial = Material(color);
        ...
        public static void Burst(Vector3 center, Color color, int count, float speed)
        {
            ...
            main.loop = false;
            main.duration = .7f;
            main.startLifetime = new ParticleSystem.MinMaxCurve(.16f, .6f);
            main.startSpeed = new ParticleSystem.MinMaxCurve(speed * .4f, speed);
            main.startSize = new ParticleSystem.MinMaxCurve(.025f, .095f);
            ...
            render.sharedMaterial = Material(color);
            render.renderMode = ParticleSystemRenderMode.Stretch;
            render.lengthScale = 2;
            ps.Emit(count);
        ...
        public static void Impact(Vector3 point, Vector3 direction, Color color, float strength)
        {
            ...
            direction.Normalize();
            var tangent = new Vector3(-direction.y, direction.x, 0);
            float s = strength * .36f;
            Beam(point - direction * s * .55f, point + direction * s, color, .09f, .08f);
            Beam(point - tangent * s * .65f, point + tangent * s * .65f, color, .05f, .07f);
            for (int i = 0; i < 4; i++)
            {
                Vector3 ray = (direction + tangent * (i - 1.5f) * .32f).normalized;
                Beam(point + ray * .06f, point + ray * (.45f + strength * .4f), color, .024f, .14f);
            }
        }
        ...
        public static void Dust(Vector3 point, Vector3 direction, float strength)
        {
            if (PresentationSettings.Effects <= 0 || EffectBudget.Active >= 40)
                return;
            for (int i = 0; i < 3; i++)
                Beam(point + new Vector3((i - 1) * .16f, .06f, -.03f), point + new Vector3((i - 1) * .33f, .08f, -.03f) + direction * strength * .18f, new Color(.48f, .57f, .57f), .035f, .16f);
        }
        ...
        public static void Afterimage(PixelActor actor, float life)
        {
            if (PresentationSettings.Effects <= 0 || EffectBudget.Active >= 48)
                return;
            var source = actor.Visual.GetComponent<SpriteRenderer>();
            var go = new GameObject("Dash silhouette");
            ...
            sprite.sharedMaterial = Resources.Load<Material>("Materials/GhostFX");
            sprite.color = new Color(.25f, .6f, .63f, .3f);
            go.AddComponent<EffectBudget>();
        ...
        public static void Glass(Vector3 center, Vector3 size)
        {
            ...
            for (int i = 0; i < 22; i++)
            {
                ...
                shard.transform.localScale = new Vector3(.035f, Random.Range(.06f, .24f), Random.Range(.08f, .28f));
                shard.GetComponent<Renderer>().sharedMaterial = Resources.Load<Material>("Materials/Glass");
                Object.Destroy(shard.GetComponent<Collider>());
                var fx = shard.AddComponent<TransientEffect>();
                fx.life = 1.8f;
                fx.velocity = new Vector3(Random.Range(-4f, 4f), Random.Range(1f, 6f), Random.Range(-3f, 3f));
                fx.fall = true;
            }
