...
namespace AfterSignal
{
    ...
    {
        VehicleArmament owner;Transform target;Vector3 aim,velocity;float age;bool shell;CityVehicle direct,launchVehicle;WorldActor source;
        ...
        public static VehicleMissile Launch(VehicleArmament owner,Vector3 start,Vector3 aim,Transform target,bool shell)
        {
            ...
            p.launchVehicle=owner?owner.GetComponent<CityVehicle>():null;p.velocity=(aim-start).normalized*(shell?340:38);
        ...
        public static VehicleMissile LaunchFaction(CityVehicle vehicle,WorldActor gunner,Vector3 start,Vector3 aim,Transform target,bool shell)
        {var missile=Launch(null,start,aim,target,shell);missile.launchVehicle=vehicle;missile.source=gunner;return missile;}
        void Update()
        {
            var g=GameDirector.Instance;if(!g||g.Blocked)return;float dt=Mathf.Min(.05f,Time.deltaTime);age+=dt;
            if(target)aim=target.GetComponent<WorldActor>()?target.GetComponent<WorldActor>().Center:target.position+Vector3.up*1.5f;
            ...
            if(Ballistics.Cast(transform.position,step.normalized,step.magnitude+.12f,launchVehicle?launchVehicle.transform:null,out var hit,source!=null)){direct=hit.collider.GetComponentInParent<CityVehicle>();transform.position=hit.point;Detonate();return;}
        ...
        void Detonate()
        {
            Detonations++;VehicleExplosion.Create(transform.position,shell?3:4);BlastDamage.Create(transform.position,shell?10:14,shell?240:270,source,launchVehicle,shell?BlastPayload.ArmourPiercing:BlastPayload.Missile,direct);
