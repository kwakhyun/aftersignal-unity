...
namespace AfterSignal
{
    ...
    public sealed class CitySocial:MonoBehaviour
    {
        ...
        bool Eligible(CityNpc n)=>n&&n.gameObject.activeInHierarchy&&!n.fixedQuest&&!n.Fleeing&&Time.time>=n.SocialUntil&&n.GetComponent<WorldActor>().Alive&&!n.GetComponent<MedicalPending>()&&n.GetComponent<SpriteRenderer>()&&n.GetComponent<SpriteRenderer>().enabled;
        void Update()
        {
            if(!game||!game.Ready||game.Blocked||game.stage!=StageId.UrbanCity||Time.time<next)return;
            next=Time.time+Random.Range(4,7);
            var people=FindObjectsByType<CityNpc>().Where(n=>Eligible(n)&&Vector3.Distance(n.transform.position,game.Player.transform.position)<48).ToArray();
            ...
            var a=people[Random.Range(0,people.Length)];
        ...
        IEnumerator Reply(CityNpc a,CityNpc b,string text)
        {
            ...
            if(b&&b.GetComponent<WorldActor>().Alive&&!b.Fleeing&&a&&!a.Fleeing)NpcSpeech.Say(b,text,4,1);
        }
