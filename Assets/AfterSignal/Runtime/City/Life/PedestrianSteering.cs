...
namespace AfterSignal
{
    ...
    public sealed class PedestrianSteering:MonoBehaviour
    {
        ...
        public Vector3 Move(Vector3 target,float distance)
        {
            ...
            if(!NpcGroundSupport.Floor(point,at.y+.5f,1.1f,out float floor)||Mathf.Abs(floor-at.y)>.48f)return Vector3.zero;
            if(!Clear(at,direction,distance+.04f))return Vector3.zero;
            point.y=floor+.035f;transform.position=point;return point-at;
        }
