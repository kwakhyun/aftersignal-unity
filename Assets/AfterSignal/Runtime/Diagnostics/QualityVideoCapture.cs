...
namespace AfterSignal
{
    ...
    public sealed class QualityVideoCapture : MonoBehaviour
    {
        ...
        IEnumerator Start()
        {
            string ffmpeg = QualitySession.Arg("-quality-ffmpeg");
            ...
            var start = new ProcessStartInfo(ffmpeg, "-y -f rawvideo -pix_fmt rgb24 -s 1600x900 -r 30 -i pipe:0 -vf vflip -an -c:v libx264 -preset ultrafast -crf 20 -pix_fmt yuv420p \"" + Path.Combine(QualitySession.Output, "gameplay-unedited.mp4") + "\"")
            {
            ...
            encoder.ErrorDataReceived += (s, e) =>
            {
            };
            encoder.BeginErrorReadLine();
            input = encoder.StandardInput.BaseStream;
            next = Time.realtimeSinceStartup;
            while (!finished)
            {
                yield return new WaitForEndOfFrame();
                if (finished)
                    yield break;
                if (Time.realtimeSinceStartup + .001f < next)
                    continue;
                ...
                int repeats = Mathf.Clamp(Mathf.FloorToInt((now - next) / Interval) + 1, 1, 8);
                for (int i = 0; i < repeats; i++)
                    input.Write(data, 0, data.Length);
                next += repeats * Interval;
            }
        }
