...
namespace AfterSignal
{
    ...
    public sealed class QualitySession : MonoBehaviour
    {
        ...
        long maxMemory = -1, maxBatches = -1;
        float started;
        QualityAudioCapture audioCapture;
        QualityVideoCapture videoCapture;
        bool lightweight;
        ...
        void Awake()
        {
            Output = Path.GetFullPath(Arg("-quality-output", Path.Combine(Application.dataPath, "../../../Artifacts/Quality/Run")));
            ...
            started = Time.realtimeSinceStartup;
            string recordStages = Arg("-quality-record-stages");
            bool record = string.IsNullOrEmpty(recordStages) || Array.IndexOf(recordStages.Split(','), GameDirector.Instance.stage.ToString()) >= 0;
            if (record && !string.IsNullOrEmpty(Arg("-quality-ffmpeg")))
            {
                audioCapture = Camera.main.gameObject.AddComponent<QualityAudioCapture>();
                videoCapture = gameObject.AddComponent<QualityVideoCapture>();
            }
        }

        void LateUpdate()
        {
            if (Time.realtimeSinceStartup - started < 2)
                return;
            frames.Add(Time.unscaledDeltaTime * 1000);
            allocations.Add(gc.Valid ? gc.LastValue : -1);
            maxMemory = Math.Max(maxMemory, memory.Valid ? memory.LastValue : -1);
            if (batches.Valid && batches.LastValue > 0)
                maxBatches = Math.Max(maxBatches, batches.LastValue);
            if (!lightweight)
            {
                FrameTimingManager.CaptureFrameTimings();
                if (FrameTimingManager.GetLatestTimings(1, timing) > 0 && timing[0].gpuFrameTime > 0)
                    gpu.Add(timing[0].gpuFrameTime);
            }
        }

        public void Save()
        {
            if (GetComponent<FrameDiagnostics>())
                GetComponent<FrameDiagnostics>().Save(Output);
            if (videoCapture)
                videoCapture.Finish();
            if (audioCapture)
                audioCapture.Save(Output);
            if (frames.Count == 0)
                return;
            var sorted = frames.ToArray();
            Array.Sort(sorted);
            double sum = 0, alloc = 0;
            int slow = 0;
        ...
        class Metrics
        {
            public string device, cpu, api, unity, build;
            public int width, height, frames, over33ms, kills, attacks;
            public int vsync = QualitySettings.vSyncCount, targetFps = Application.targetFrameRate;
            public double displayHz = Screen.currentResolution.refreshRateRatio.value;
            public double meanMs, p95Ms, p99Ms, meanGcBytes, gpuMeanMs;
            public long peakMemoryBytes, maxBatches;
            public float health;
        }
    }
}
