...
with zipfile.ZipFile(destination) as archive:
    ...
    required = ["PLAY.cmd", "OPEN_UNITY.cmd", "Packages/manifest.json", "ProjectSettings/ProjectVersion.txt", "Assets/AfterSignal/Runtime/PlayerMotor.cs", "Assets/AfterSignal/Runtime/RopeMotor.cs", "Assets/AfterSignal/Scenes/01_NeonStation.unity", "Builds/Windows/AFTERSIGNAL.exe", "Documentation/VERIFICATION.md"]
    assert all("AFTERSIGNAL-Unity/" + name in archive.namelist() for name in required)
    assert all("AFTERSIGNAL-Unity/"+name in archive.namelist() for name in ["Assets/AfterSignal/Scenes/17_Origin.unity","Assets/AfterSignal/Scenes/18_Harbor.unity","Assets/AfterSignal/Runtime/CampaignCatalog.cs","Documentation/Expansion/VERIFICATION.md","Documentation/Expansion/verification.json"])
